    ...
    $('#trxusers').dataTable({
        ...
        columns: [
            ...
            { 
                ...
                name: 'lunas',
                render: function(data, type, rows, meta) {
                    return /*html*/`<a href="{{ url('laporan/cek/lfw/apply') }}/${rows.id}" class-confirm="btn-warning" ${(rows.lunas) ? 'disabled' : '' } class="btn btn-xs btn-success ${(rows.lunas) ? 'disabled' : 'confirm'}">${(rows.lunas) ? 'Sudah Lunas' : 'Apply'}</a>`;
                    return '';
                },
            },
            { 
